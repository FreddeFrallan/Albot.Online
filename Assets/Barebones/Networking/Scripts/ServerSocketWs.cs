...
namespace Barebones.Networking
{
    ...
    public class ServerSocketWs : IServerSocket, IUpdatable
    {
        ...
        public class WsService : WebSocketBehavior
        {
            ...
            private Queue<byte[]> _TCPMessageQueue;
            ...
            public WsService(ServerSocketWs serverSocket)
            {
                ...
                _TCPMessageQueue = new Queue<byte[]>();
            }

            private void Update()
            {
                if (_TCPMessageQueue.Count <= 0)
                    return;

                lock (_TCPMessageQueue)
                {
                    // Notify about new messages
                    while (_TCPMessageQueue.Count > 0)
                    {
                        if (OnMessageEvent != null)
                        {
                            OnMessageEvent.Invoke(_TCPMessageQueue.Dequeue());
                        }
                    }
                }
            }
            ...
            protected override void OnMessage(MessageEventArgs e)
            {
                lock (_TCPMessageQueue)
                {
                    _TCPMessageQueue.Enqueue(e.RawData);
                }
            }
