...
//		private string currentVersion = "0.2";
		private bool isCorrectVersion = true;
		public WrongVersionController versionController;


        void Awake(){
			if (ClientUI.ClientUIOverlord.hasLoaded)Destroy(this.gameObject);
            if (_instance != null){
                Destroy(gameObject);
                return;
            }
		...
		private void handleCurrentVersionMsg(IIncommingMessage message){
			/*
			StringMsg msg = message.Deserialize<StringMsg> ();
			if (msg.msg != currentVersion)
				handleNotValidVerison ();
			*/
		}

		private void handleNotValidVerison(){
			versionController.activateWrongVersionPanel ();
