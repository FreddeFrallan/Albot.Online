...
namespace Barebones.MasterServer{
    public class MsfArgs{
		...
		#region Init Queue
		private static List<Action> postInitFuncs = new List<Action> ();
		private static bool argsInited = false;
		public static void runAfterArgsInit(Action a){
			if (argsInited)
				a.Invoke ();
			else
				postInitFuncs.Add (a);
		...
		private static void broadcastInitArgs(){
			argsInited = true;
			foreach (Action a in postInitFuncs)
				a.Invoke ();
		}
		#endregion
