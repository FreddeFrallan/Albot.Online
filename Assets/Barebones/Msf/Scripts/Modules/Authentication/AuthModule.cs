...
namespace Barebones.MasterServer
{
    ...
    public class AuthModule : ServerModuleBehaviour
    {
      ...
      //     server.SetHandler((short)MsfOpCodes.GetPeerAccountInfo, HandleGetPeerAccountInfo);
		...
		public string getKeyFromUser(IPeer peer){
			foreach (KeyValuePair<string, IUserExtension> u in LoggedInUsers) {
				if (u.Value.Peer.Id == peer.Id){
					return u.Value.AccountData.Properties["AlphaKey"];
				}
			}
			return null;
		}
