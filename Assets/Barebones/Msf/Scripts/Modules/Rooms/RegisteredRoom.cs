...
namespace Barebones.MasterServer{
    ...
    public class RegisteredRoom{
		...
		private Dictionary<int, IPeer> playersForSpectator = new Dictionary<int, IPeer> ();
		...
		private void onPlayerJoined(IPeer p){
			if (playersForSpectator.ContainsKey (p.Id) == false)
				playersForSpectator.Add (p.Id, p);
		}
		private void onPlayerLeft(IPeer p){
			if (playersForSpectator.ContainsKey (p.Id))
				playersForSpectator.Remove (p.Id);
		}


		public Dictionary<int, IPeer> getCurrentPlayers(){return playersForSpectator;}
		#endregion
    }
}