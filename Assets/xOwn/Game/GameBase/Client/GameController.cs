...
namespace ConnectFour
{
	public class GameController : MonoBehaviour 
	{
		...
		public bool playMove(int moveNumber){
			if (gameObjectTurn == null)
				return false;

			gameObjectTurn.transform.position = new Vector3(Mathf.Clamp(moveNumber, 0, numColumns-1), gameObjectField.transform.position.y + 1, 0);

			List<int> possibleMoves = GetPossibleMoves ();
			if (possibleMoves.Contains (Mathf.RoundToInt (gameObjectTurn.transform.position.x)) == false)
				return false;
